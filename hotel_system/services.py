...
# !/usr/bin/env python
"""
# este modulo define los servicios para
# gestionar hoteles, clientes y reservas.
"""
...
class HotelService:
    ...
    incluyendo CRUD y reservas. """
    def __init__(self):
        """ Inicializa el servicio de hoteles
        con un almacenamiento JSON. """
        self.storage = JsonStorage("data/hotels.json")

    def create_hotel(self, hotel: Hotel):
        """ Crea un nuevo hotel y
        lo guarda en el almacenamiento. """
    ...
    def delete_hotel(self, hotel_id):
        ...
        actualizando el almacenamiento. """
        hotels = self.storage.load()
        hotels = [h for h in hotels if h["hotel_id"] != hotel_id]
        self.storage.save(hotels)

    def get_hotel(self, hotel_id):
        """ Obtiene un hotel por su ID, devolviendo
        su informacion o None si no existe. """
        hotels = self.storage.load()
    ...
    def reserve_room(self, hotel_id):
        """ Reserva una habitacion en un hotel por su ID,
    ...
    def cancel_reservation(self, hotel_id):
        """ Cancela una reserva en un hotel por su ID,
        actualiza las habitaciones. """
        hotels = self.storage.load()
        for hotel in hotels:
            if hotel["hotel_id"] == hotel_id:
...
class CustomerService:
    ...
    def get_customer(self, customer_id):
        """ Obtiene un cliente por su ID, devolviendo
        su informacion o None si no existe. """
        customers = self.storage.load()
        return next((c for c in customers
                     if c["customer_id"] == customer_id), None)

    def update_customer(self, customer_id, **kwargs):
        """ Actualiza la informacion de un cliente
        por su ID, actualzia informacion """
        customers = self.storage.load()
...
class ReservationService:
    ...
    incluyendo creacion y cancelacion. """
    def __init__(self):
